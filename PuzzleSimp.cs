...
namespace Will
{
    public partial class PuzzleSimp : Form
    {
        ...
        private void btnPlayAgain(object sender, EventArgs e)
        {
            ...
            {
                YesOrNo = MessageBox.Show("Are You Sure To RESTART ?", "Simpsons Puzzle", MessageBoxButtons.YesNo, MessageBoxIcon.Question); // when a user clicks shuffle while timer has strater they will be prompted if they would like to start again
            }
        ...
        {
            DialogResult YesOrNO = MessageBox.Show("Are You Sure To Quit ?", "Simpsons Puzzle", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
        ...
        private void PuzzleSimp_Load(object sender, EventArgs e)
        {
            Randomize();
        }
            ...
            if (inNullSliceIndex != inPictureBoxIndex)// if null pictuer box image is not equal to the pictuerbox index code below will execte
            {
                ...
                {// if current pictuer box and blank pictuer box are neighbous images can be swaped
                    ...
                    if (CheckPuzzle()) //checking if it is a win state then timer will stop
                    {
                        ...
                        MessageBox.Show("Congratulations...\nYour Simpsons  family Is Happy\nTime Elapsed : " + timer.Elapsed.ToString().Remove(8) + "\nTotal Moves Made : " + inmoves, "Simpsons Puzzle");
